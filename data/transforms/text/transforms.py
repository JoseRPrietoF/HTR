...
class ToTensor(object):
    ...
    def __call__(self, x):
        #return self._syms[c] for c in x #TODO rehacer
        res = []
        for word in x:
            res.append(self._syms[word])
        #     word = word.lower()
        #     for c in word:
        #         res.append(self._syms[c])
        #         #print("{} -> {}".format(c, self._syms[c]))
        #     res.append(self._syms['<space>'])
        #     #print("<space> -> ", self._syms['<space>'])
        # res.pop()
        return res
