...
class CTCPrepare(Function):
    ...
    def backward(ctx, grad_output, *_):
        #print(ctx)
        #check(grad_output, inp="'grad_output' -> backward input")
        #print(grad_output)
...
class CTCLoss(Loss):
    ...
    def add_logsoftmax(self, value):
        ...
        if self._implementation == CTCLossImpl.PYTORCH and not self._add_logsoftmax:
            print(_WARNING_PYTORCH_LOGSOFTMAX)
    ...
    def implementation(self, value):
        ...
        if value not in (CTCLossImpl.PYTORCH, CTCLossImpl.BAIDU):
            ...
            if torch_baidu_ctc is None:
                print(_WARNING_BAIDU_NOT_AVAILABLE)
                value = CTCLossImpl.PYTORCH
            else:
                print(_WARNING_BAIDU_WRONG)
    ...
    def forward(self, output, target, **kwargs):
        ...
        if not valid_indices:
            print("All samples in the batch were ignored!")
            return None
        ...
        if self._add_logsoftmax:
            #check(acts, inp="Before log_softmax")
